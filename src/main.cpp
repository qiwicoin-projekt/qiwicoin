...
#if defined(NDEBUG)
# error "Qiwicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1050e08e67db9451b964f3c2a6fb78c25ef1dbafa246c41c3a11fb537c223f29");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Qiwicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Qiwicoin Signed Message:\n";
...
{
    // Qiwicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
if(nHeight == 1)   
		{ nSubsidy = 7000000000 * COIN; }
    if (nHeight >= 1)
	{
        for (int qiwi = 0; qiwi < nHeight /10; qiwi++)
		{ nSubsidy = (nSubsidy/99)*93; }
	}
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 30; 
static const int64 nTargetSpacing = 3 * 30; 
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Qiwicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Qiwicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1050e08e67db9451b964f3c2a6fb78c25ef1dbafa246c41c3a11fb537c223f29");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "this is genesisblock";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042284710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1504908600;
        ...
        assert(block.hashMerkleRoot == uint256("0xce0c63f1ad1071b03313d744c8832721394f0e0d0213876ce17cb435b67ee163"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Qiwicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("QiwicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("QiwicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static QiwicoinMiner(CWallet *pwallet)
{
    printf("QiwicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("qiwicoin-miner");
    ...
    try { loop {
        ...
        printf("Running QiwicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("QiwicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&QiwicoinMiner, pwallet));
}
